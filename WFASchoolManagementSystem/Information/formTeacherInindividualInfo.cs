...
namespace WFASchoolManagementSystem.Information
{
    public partial class formTeacherInindividualInfo : Form
    {
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = "select type from AllUser where password = '" + this.textBoxPassword.Text + "';";
                DataAccess da = new DataAccess();
                DataSet ds = da.ExecuteQuery(sql);

                if (ds.Tables[0].Rows.Count == 1)
                {
                    string gdvsql = @"SELECT AllUser.userId, 
                         Teacher.fullname, 
                         Teacher.email, 
                         Teacher.gender, 
                         Teacher.dateofbirth,
                         ...
                         FROM AllUser
                         INNER JOIN Teacher ON AllUser.userId=Teacher.userId;";



                    this.PopulateGridView(gdvsql);
                }
                else
                {
